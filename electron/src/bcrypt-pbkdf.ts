...
// bcrypt_pbkdf, the key derivation OpenSSH private key files use when they
// are passphrase-protected. Node's crypto has no implementation, so this is
// a port of OpenBSD's blowfish.c and bcrypt_pbkdf.c, taken from the
// bcrypt-pbkdf npm package (Devi Mandiri's JavaScript port of the same two
// files) with its tweetnacl SHA-512 swapped for Node's. The S-boxes and the
// P-array below are copied from that source unchanged.
//
// Blowfish: Copyright 1997 Niels Provos, BSD-3-Clause.
// bcrypt_pbkdf: Copyright (c) 2013 Ted Unangst, ISC.

const S_INIT: readonly (readonly number[])[] = [
    [
...
class Blowfish {
    ...
    readonly P = Uint32Array.from(P_INIT);
    // Byte cursor into the key and data streams, shared across the expansion
    // steps the way blf_J is in the C
    private j = 0;

    private f(x8: Uint8Array, i: number): number {
        const S = this.S;
        return (((S[0][x8[i + 3]] + S[1][x8[i + 2]]) ^ S[2][x8[i + 1]]) + S[3][x8[i]]) >>> 0;
    }

    // x8 is a byte view over x, so the S-box lookups read the words the C
    // reads: host order, which for this port is always little-endian because
    // the Uint32Array carries it (the S-box indexing only has to agree with
    // the byte order the words are written back in, and it does)
    encipher(x: Uint32Array, x8: Uint8Array): void {
        const P = this.P;
        x[0] ^= P[0];
        for (let i = 1; i < 16; i += 2) {
            x[1] ^= this.f(x8, 0) ^ P[i];
            x[0] ^= this.f(x8, 4) ^ P[i + 1];
        }
        const t = x[0];
        x[0] = x[1] ^ P[17];
        x[1] = t;
    ...
    expand0state(key: Uint8Array, keybytes: number): void {
        const d = new Uint32Array(2);
        const d8 = new Uint8Array(d.buffer);
        this.j = 0;
        for (let i = 0; i < 18; i++) this.P[i] ^= this.stream2word(key, keybytes);
        this.j = 0;
    ...
    // The magic is read through the same cursor as the key material, right
    // after the expansion loop leaves it at zero
    words(data: Uint8Array, count: number): Uint32Array {
        const out = new Uint32Array(count);
        for (let i = 0; i < count; i++) out[i] = this.stream2word(data, data.byteLength);
        return out;
    }

    enc(data: Uint32Array, blocks: number): void {
        for (let i = 0; i < blocks; i++) {
            const block = data.subarray(i * 2, i * 2 + 2);
            this.encipher(block, new Uint8Array(block.buffer, block.byteOffset, 8));
        }
    }
}
...
const MAGIC = Buffer.from('OxychromaticBlowfishSwatDynamite', 'latin1');
...
function bcryptHash(sha2pass: Uint8Array, sha2salt: Uint8Array, out: Uint8Array): void {
    ...
    const cdata = state.words(MAGIC, BCRYPT_BLOCKS);
    for (let i = 0; i < 64; i++) state.enc(cdata, BCRYPT_BLOCKS / 2);
...
export async function bcryptPbkdf(pass: Uint8Array, salt: Uint8Array, keylen: number, rounds: number): Promise<Uint8Array> {
    ...
    if (pass.length === 0 || salt.length === 0 || keylen === 0
        ...
        throw new Error('bcrypt_pbkdf: invalid parameters');
    }
    const key = new Uint8Array(keylen);
    const out = new Uint8Array(BCRYPT_HASHSIZE);
    const tmpout = new Uint8Array(BCRYPT_HASHSIZE);
    const countsalt = new Uint8Array(salt.length + 4);
    countsalt.set(salt);
    const sha2pass = sha512(pass);
    ...
    for (let count = 1; remaining > 0; count++) {
        countsalt[salt.length] = count >>> 24;
