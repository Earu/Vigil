...
export class BreachStatusStore {
    ...
    public static setEntryStatus(databasePath: string, entryId: string, status: { isPwned: boolean; count: number }): void {
    ...
    public static getEntryStatus(databasePath: string, entryId: string): { isPwned: boolean; count: number } | null {
        const store = this.getStore();
