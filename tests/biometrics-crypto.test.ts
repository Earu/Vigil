...
describe('mac-session envelope', async () => {
    const c = await import('../electron/src/biometrics-crypto');
    const keychainKey = randomBytes(32);

    it('round-trips under the key the keychain item holds', () => {
        const sealed = c.sealSessionWithKeychainKey('hunter2', keychainKey);
        expect(sealed.startsWith('mac-session:')).toBe(true);
        expect(c.openSessionWithKeychainKey(sealed, keychainKey)).toBe('hunter2');
    });

    it('rejects a wrong key (the item was regenerated by a later arming)', () => {
        const sealed = c.sealSessionWithKeychainKey('hunter2', keychainKey);
        expect(() => c.openSessionWithKeychainKey(sealed, randomBytes(32))).toThrow();
    });

    // The two macOS scopes read the same keychain item, so the domain
    // separation in the HKDF info is what stops a session copy that somehow
    // reached disk from being opened as a persistent one, and the other way
    it('does not cross-open with the persistent scope on the same key', () => {
        const session = c.sealSessionWithKeychainKey('hunter2', keychainKey);
        const persistent = sealWithKeychainKey('hunter2', keychainKey);

        expect(isV3Blob(session)).toBe(false);
        expect(session.startsWith('v3:')).toBe(false);
        expect(() => openWithKeychainKey(session, keychainKey)).toThrow();
        expect(() => c.openSessionWithKeychainKey(persistent, keychainKey)).toThrow();
    });

    it('uses a fresh IV per seal, so the same password never repeats a blob', () => {
        expect(c.sealSessionWithKeychainKey('pw', keychainKey))
            .not.toBe(c.sealSessionWithKeychainKey('pw', keychainKey));
    });

    it('rejects a truncated blob', () => {
        expect(() => c.openSessionWithKeychainKey('mac-session:' + randomBytes(8).toString('base64'), keychainKey))
            .toThrow('Malformed biometric blob');
    });
});
